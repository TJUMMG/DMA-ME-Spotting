...
def get_spotted_intervals_for_video(video_name, window_len, kl, kr, peakl, peakr, p):
    ...
    Cmean = np.sum(Carr[2*kl:len(Carr)-2*kr])/len(Carr)
    Cmax = np.max(Carr[2*kl:len(Carr)-2*kr])
    epsilon = 0.01 * p
    # threshold
    ...
    for i in range(2*kl, len(Carr)-2*kr):
        if (Carr[i] >= Thr):
            res[i] = 1
            j = i + 1
            k = i - 1
...
def Evaluate_CASme2(window_len, kl, kr, peakl, peakr, p):
    ...
    for sub_folder in subfolders:
        ...
        for vidname in vidfolders:
            ...
            spot_interval_list = get_spotted_intervals_for_video(vidname, window_len, kl, kr, peakl, peakr, p)
...
def main():
    Evaluate_CASme2(window_len=11, kl=4, kr=6, peakl=3, peakr=4, p=11)
    # Evaluate_CASme2(window_len=11, kl=6, kr=4, peakl=4, peakr=3, p=9)
    # Evaluate_CASme2(window_len=15, kl=6, kr=8, peakl=4, peakr=5, p=6)
    # Evaluate_CASme2(window_len=15, kl=8, kr=6, peakl=5, peakr=4, p=21)
