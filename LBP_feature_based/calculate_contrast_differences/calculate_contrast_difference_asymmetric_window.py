...
def calculate_contrast_difference(dataset, video_file, window_len, kl, kr):
    print("Starting !!")
...
def save_contrast_differences_for_CASme2(window_len, kl, kr):
    rst_path = r'..\..\..\results\contrast differences\CAS(ME)^2_LBP_contrast_differences' + '\\' + 'window_len_' + str(window_len) + '_kl_' + str(kl) + '_kr_' + str(kr)
    ...
    for sub_folder in subfolders:
        ...
        for vidname in vidfolders:
            print(vidname)
            Farr, Carr = calculate_contrast_difference('CASme2', os.path.join(folder_data, sub_folder, vidname), window_len, kl, kr)
...
def save_contrast_difference_for_SAMM_Long_Videos(window_len, kl, kr):
    rst_path = r'..\..\..\results\contrast differences\SAMM_Long_Videos_LBP_contrast_differences' + '\\' + 'window_len_' + str(window_len) + '_kl_' + str(kl) + '_kr_' + str(kr)
    if not os.path.exists(rst_path):
        os.mkdir(rst_path)
    ...
    for vidname in vidfolders:
        print(vidname)
        Farr, Carr = calculate_contrast_difference('SAMM Long Videos', os.path.join(folder_data, vidname), window_len, kl, kr)
...
def main():
    # save_contrast_differences_for_CASme2(window_len=11, kl=4, kr=6)
    # save_contrast_differences_for_CASme2(window_len=11, kl=6, kr=4)
    # save_contrast_differences_for_CASme2(window_len=15, kl=6, kr=8)
    # save_contrast_differences_for_CASme2(window_len=15, kl=8, kr=6)
    save_contrast_difference_for_SAMM_Long_Videos(window_len=57, kl=22, kr=34)
    # save_contrast_difference_for_SAMM_Long_Videos(window_len=57, kl=34, kr=22)
    # save_contrast_difference_for_SAMM_Long_Videos(window_len=89, kl=38, kr=50)
    # save_contrast_difference_for_SAMM_Long_Videos(window_len=89, kl=50, kr=38)
